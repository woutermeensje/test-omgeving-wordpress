...
(function (window, document, $, wpgetapi, undefined) {
	...
	wpgetapi.onReady = function () {
		...
		$page
			...
			.on( 'cmb2_shift_rows_complete', wpgetapi.shiftAPIName );
	...
	/**
	 * Shift API Name value.
	 *
	 * @param {Event}  e    - The event object.
	 * @param {jQuery} from - The element from which the row is moved.
	 * @param {jQuery} goto - The element to which the row is moved.
	 */
	wpgetapi.shiftAPIName = function ( e, from, goto ) {
		if ( goto.find( '.cmbhandle-title span' ).length ) {
			var $destinationElement     = from.hasClass( 'move-up' ) ? goto.prev( '.cmb-repeatable-grouping' ) : goto.next( '.cmb-repeatable-grouping' );
			var currentElementTitle     = goto.find( '.cmbhandle-title span' ).text();
			var destinationElementTitle = $destinationElement.find( '.cmbhandle-title span' ).text();

			goto.find( '.cmbhandle-title span' ).text( destinationElementTitle );
			$destinationElement.find( '.cmbhandle-title span' ).text( currentElementTitle );
		}
	}
